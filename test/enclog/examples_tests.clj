...
wraps a call to your real fitness-function (like here) seems a good choice." 
  [net verbose?]
  (.scorePilot ^NeuralPilot (NeuralPilot. net verbose?)))
...
(defn predict-sunspot 
  ...
  (let [start-year  1700
    ...
    (do (train trainer []) ;;SVM TRAINED AND READY FOR PREDICTIONS AFTER THIS LINE
        ...
        (loop [evaluation-start (int (inc train-end))]
          (if (== evaluation-start evaluation-end)
            (println "DONE...!")
            (let [^MLData input  (data :basic window-size)]
              ...
              (recur (unchecked-inc-int evaluation-start))))))))
;--------------------------------------------------------------------------------------------------------------
;--------------------------------*SIMPLE SOM EXAMPLE*----------------------------------------------------------
